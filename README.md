...
通过串口 DTR/RTS 键控发报，可选 Yaesu / Icom CAT 读频，ADIF 日志追加写入，内置 Big CTY 国际冠字表做呼号前缀解析。

## 功能概览

| 模块 | 说明 |
|------|------|
| **键控发报** | 发送框输入即发；5–40 WPM 可调；Esc 停止发送并清空发送框 |
...
|------|------|
| `Alt+K` | 聚焦摩尔斯发送框 |
| `Alt+C` / `Alt+N` / `Alt+Q` | 将对方呼号 / 姓名 / QTH 追加到发送框（空则跳过） |
